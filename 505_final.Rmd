...
vars_nn <- vars_nn %>%
  select(POST_ID, dotw, spc_event.nn, reg_business.nn, lights.nn, enforcement.nn, school.nn, college.nn, transitstop.nn, historic_resources.nn, reg_historical.nn) %>%
  st_transform(crs = "+proj=longlat +crs = 'EPSG:6339'")
...
dat_all <- read.delim("dat_all_01.txt")
...
park_exp.nn <-
  dat_all %>%
  dplyr::select(pk_Count,
                spc_event.nn, reg_business.nn, lights.nn, enforcement.nn, school.nn, college.nn, transitstop.nn, historic_resources.nn, reg_historical.nn,
    -LONGITUDE, -LATITUDE, -geometry, -geometry.x, -geometry.y) %>%
  gather(key,value, -pk_Count)
...
### Backward selection
```{r backward}
olsModel_backstep <- step( lm(formula = pk_Count ~ ., data = final_vars), direction="backward") #backstep
olsModel1 <- lm ( pk_Count ~ Total_Pop + Percent_White + Percent_Taking_Public_Trans + 
    spc_event.nn + lights.nn + enforcement.nn + transitstop.nn + 
    historic_resources.nn + ON_OFFSTREET_TYPE + Neighborhoods + 
    cultural + medical + dotw, data = final_vars) 
summary(olsModel1)

vif(olsModel) #spc_event.nn  6.943247 1.000000 2.635004  #lights.nn  5.614941 1.000000 2.369587
sqrt(vif(olsModel))
...
model.names=c("A: y=Log Total","B: y=Total","C: y=Log Res Burden"),
note=c("Table 18. Model Comparison Table across all 'needs' ")) %>%
set_width(0.9) %>%
set_row_height(0.5) %>%
set_font_size(8) %>%
set_all_padding(0.5)
...
## Probabilities versus changes in key variables

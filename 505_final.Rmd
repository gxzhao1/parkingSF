...
dat_all$cat_age <- ifelse(dat_all$Med_Age <= 35.9, "1QU", ifelse(dat_all$Med_Age > 35.9 & dat_all$Med_Age <= 37.3, "2QU", ifelse(dat_all$Med_Age > 37.3 & dat_all$Med_Age <= 41.9, "3QU", "4QU"))) #categorize age
  ...
  # 19.60   35.90   37.30   39.38   41.90   64.70 
dat_all$cat_inc <- ifelse(dat_all$Med_Inc <= 63808, "1QU", ifelse(dat_all$Med_Inc > 63808 & dat_all$Med_Inc <= 98531, "2QU", ifelse(dat_all$Med_Inc > 98531 & dat_all$Med_Inc <= 141588, "3QU", "4QU"))) #categorize income
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  # 12479   63808   98531   99825  141588  195000 
```
...
park_exp.cat <-
  dat_all %>%
  dplyr::select(pk_Count,
                ON_OFFSTREET_TYPE, Neighborhoods, Means_of_Transport, pdr, cultural, medical, management, cat_inc, cat_age,#visitor #retail, 
    -LONGITUDE, -LATITUDE, -geometry, -geometry.x, -geometry.y) %>%
  gather(key,value, -pk_Count)
...
final_vars <- dat_all %>%
  select(pk_Count,
        ...
        Total_Pop, 
        Percent_White, Percent_Taking_Public_Trans, spc_event.nn, 
        ...
        ON_OFFSTREET_TYPE, pdr, cat_inc, cat_age, #Med_Age, 
        cultural, medical, management, dotw#, lights.nn
         )
...
final_numeric_vars <- dat_all %>%
  select(pk_Count,
         # continuous
        Total_Pop, 
        Percent_White, Percent_Taking_Public_Trans, spc_event.nn, #lights.nn,
        enforcement.nn, transitstop.nn, historic_resources.nn, nearby.nn)
...
#### OLS
```{r OLS no log}
olsModel <- lm ( pk_Count ~ ., data = final_vars) #before cat 0.2405 #after cat 0.2493(i forgot to delete med-age the first time) Adjusted R-squared:  0.2484 
summary(olsModel)

```

```{r log level}
olsModel.loglevel <- lm ( log(pk_Count) ~ ., data = final_vars) #0.2119 before cat #0.222 after cat(now it's 0.22)
...
#           # retail, visitor, due to only one value
#           # Means_of_Transport, due to too many categories
#         ON_OFFSTREET_TYPE, pdr, #Neighborhoods, 
#         cultural, medical, management, dotw, nearby.nn
#          )

final_vars.logsome <- final_vars %>%
  mutate(log_Percent_Taking_Public_Trans = log(Percent_Taking_Public_Trans)) %>%
  select(-Percent_Taking_Public_Trans)
  

olsModel.loglog <- lm ( log(pk_Count) ~ ., data = final_vars.logsome) #0.2122 before cat #0.2224 after cat (0.22)
...
binomialModel <- glm(as.factor(highDemand) ~.,family = "binomial"(link="logit"),
                  data= binomial_vars)
summary(binomialModel) # AIC 174264 #Null deviance: 205298 #Residual deviance: 174220 #AFTER CAT AIC: 173080 (AIC: 173258) Residual deviance: 173026(173206)
```
...
olsModel1 <- lm ( pk_Count ~ Total_Pop + Percent_White + Percent_Taking_Public_Trans + 
    spc_event.nn + enforcement.nn + transitstop.nn + historic_resources.nn + 
    nearby.nn + ON_OFFSTREET_TYPE + cat_inc + cat_age + cultural + 
    medical + dotw, data = final_vars)  # got rid of management
#Step:  AIC=4598.67
#Step after cat: AIC=4598.67(AIC=4800.22) Adjusted R-squared:  0.2494 (0.2484)
summary(olsModel1)
...
# Got rid of pdr, management based on backward selection and drop1 functions
binomialModel1 <- glm(formula = as.factor(highDemand) ~ Total_Pop + Percent_White + 
    Percent_Taking_Public_Trans + spc_event.nn + enforcement.nn + 
    transitstop.nn + historic_resources.nn + nearby.nn + ON_OFFSTREET_TYPE + 
    cat_inc + cat_age + cultural + medical + dotw, family = binomial(link = "logit"), 
    data = binomial_vars)
summary(binomialModel1)
#     Null deviance: 205298  on 156522  degrees of freedom
# Residual deviance: 173208  on 156499  degrees of freedom
# AIC: 173256
...
#157443.4                      # Residual deviance: 173206  on 156497  degrees of freedom
#157418.4                      # AIC: 173258
...
# [1] 157443.4
# [1] 157420.4
anova(binomialModel1, test="Chisq")
drop1(binomialModel1, test="Chisq")
...
dotw_predictions <- 
  dat_all %>% 
    mutate(olsmodel = map(.x = data, fit = olsModel1, .f = model_pred)) %>% 
    gather(Regression, Prediction, -data) %>%
    mutate(Observed = map(data, pull, pk_Count),
...
dotw_predictions %>% 
    mutate(dotw= map(data, pull, dotw),
           POST_ID = map(data, pull, POST_ID)) %>%
    dplyr::select(dotw, POST_ID, Observed, Prediction, Regression) %>%
    unnest() %>%
    gather(Variable, Value, -Regression, -dotw, -POST_ID) %>%
    group_by(Regression, Variable, dotw) %>%
    summarize(Value = sum(Value)) %>%
    ggplot(aes(dotw, Value, colour=Variable)) + 
      geom_line(size = 1.1) + 
      facet_wrap(~Regression, ncol=1) +
      labs(title = "Figure 5.2: Predicted/Observed parking demands time series", x = "dotw", y= "Meter Locations") +
      plotTheme
```
...
dotw_predictions %>% 
    ...
    dplyr::select(dotw, POST_ID, LONGITUDE, LATITUDE, Observed, Prediction, Regression) %>%
    unnest() %>%
  #filter(Regression == "DTime_Space_FE_timeLags") %>% ##regresstion
  group_by(POST_ID, LONGITUDE, LATITUDE) %>%
  summarize(MAE = mean(abs(Observed-Prediction), na.rm = TRUE))%>%
ggplot(.)+
  geom_sf(data = (SFTracts%>%
    st_transform(st_crs(6339))), color = "grey", fill = "transparent")+

...
#write.table(dat_all, sep="\t", file="dat_all_01.txt", row.names=FALSE)
...
olsModel <- lm ( pk_Count ~ ., data = final_vars %>% select(-Neighborhoods)) 
summary(olsModel)
```
...
binomialModel1 <- glm(formula = as.factor(highDemand) ~ Total_Pop + Med_Age + Percent_White + 
    ...
    transitstop.nn + historic_resources.nn + ON_OFFSTREET_TYPE + 
    Neighborhoods + cultural + medical + dotw, family = binomial(link = "logit"), 
    data = binomial_vars)
...
# The best threshold is 0.325

testProbs <- 
  testProbs %>%
  mutate(predOutcome  = as.factor(ifelse(testProbs$Probs > 0.325 , 1, 0)))

caret::confusionMatrix(testProbs$predOutcome, testProbs$Outcome, 
                       positive = "1")

pROC::auc(testProbs$Outcome, testProbs$Probs)
# Area under the curve is 0.7572

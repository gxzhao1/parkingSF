...
setwd("D:/Upenn/CPLN505/final project/parkingSF")
```
...
dat_all <- dat_all %>%
  mutate(pk_Count = pk_Count/4) #why?
...
park_exp.cat <-
  dat_all %>%
  dplyr::select(pk_Count,
                ON_OFFSTREET_TYPE, Neighborhoods, Means_of_Transport, pdr, cultural, medical, management,
    -LONGITUDE, -LATITUDE, -geometry, -geometry.x, -geometry.y) %>%
  gather(key,value, -pk_Count)
